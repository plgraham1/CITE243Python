...
Date: 10/19/2025
Description:
    Multiple security and file management tools in a single program
...
class Sidebar(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.buttons = []
        for label in ["SEC OPS", "Regex Search", "File Tools", "Builder", "Link Verifier", "Google Tools"]:
            b = OpsButton(label)
            layout.addWidget(b)
...
class OpsWindow(QtWidgets.QMainWindow):
    ...
    def _on_nav(self, btn):
        ...
        if label == "SEC OPS":
            ...
            if not hasattr(self, "_googtools") or self._googtools is None:
...
class GoogleToolsPanel(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        desc = QtWidgets.QLabel(
            "Choose one of the following automated Google tasks:"
        )
    ...
    def download_form_data(self):
        import ezsheets
        self.log.append("Connecting to Google Sheets...")
        try:
            ss = ezsheets.Spreadsheet(input("Enter the Google Sheet ID for form responses: "))
            sheet = ss[0]
            emails = [row[1] for row in sheet.getRows()[1:] if row[1]]
    ...
    # --- 2. Convert spreadsheet to multiple formats ---
    def convert_spreadsheet(self):
        import ezsheets
        self.log.append("Converting Google Sheet to other formats...")
        try:
            ss = ezsheets.Spreadsheet(input("Enter Spreadsheet ID: "))
            ...
            self.log.append(f"Saved to {base}")
        except Exception as e:
            self.log.append(f"Error: {e}")

    # --- 3. Find mistakes in spreadsheet totals ---
    def find_mistakes(self):
        import ezsheets
        self.log.append("Scanning spreadsheet for math errors...")
        try:
            ...
            for rowNum in range(2, sheet.rowCount + 1):
                ...
                try:
                    ...
                    continue
            self.log.append("Check complete.")
        except Exception as e:
            self.log.append(f"Error: {e}")
